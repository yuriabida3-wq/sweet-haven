...
function getProducts() {
    return JSON.parse(localStorage.getItem('sweetHavenProducts')) || [];
}
...
function submitOrder() {
    ...
    if (phone === '') { alert('Please enter your phone number so we can contact you.'); return; }
...
function showTab(tabId) {
    ...
    document.getElementById(tabId === 'ordersTab' ? 'btnOrders' : 'btnProducts').classList.add('active');
}
...
function renderOwnerOrders() {
    ...
    if (orders.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: #888; padding: 20px;">No orders yet.</td></tr>';
    } else {
        tbody.innerHTML = orders.map((order, index) => {
            ...
            return `
            <tr>
                <td>${order.date}</td><td><strong>${order.id}</strong></td><td><strong>${order.phone}</strong></td>
                <td>${order.item}</td><td>${order.qty}</td><td>${order.type}</td>
                <td>KSh ${order.total}</td>
                <td><span class="status-badge ${order.status === 'Pending' ? 'status-pending' : 'status-completed'}">${order.status}</span></td>
                <td>${order.status === 'Pending' ? `<button class="action-btn" onclick="markCompleted(${index})">Mark Delivered</button>` : '✅ Done'}</td>
            </tr>`;
        }).join('');
    }
...
function markCompleted(index) {
    let orders = JSON.parse(localStorage.getItem('sweetHavenOrders')) || [];
    if(orders[index]) { orders[index].status = 'Completed'; localStorage.setItem('sweetHavenOrders', JSON.stringify(orders)); renderOwnerOrders(); }
}
...
function exportCSV() {
    ...
    orders.forEach(function(order) {
        csvContent += [`"${order.date}"`, `"${order.id}"`, `"${order.phone}"`, `"${order.item}"`, `"${order.qty}"`, `"${order.type}"`, `"${order.destination || 'N/A'}"`, `"${order.total}"`, `"${order.status}"`].join(",") + "\r\n";
    });
    const link = document.createElement("a"); link.setAttribute("href", encodeURI(csvContent)); link.setAttribute("download", "Sweet_Haven_Sales_Report.csv");
...
function renderDashboardProducts() {
    ...
    const products = getProducts();
    if (products.length === 0) {
        container.innerHTML = '<p style="color: #888;">No products added yet.</p>';
        return;
    }
    container.innerHTML = products.map(prod => `
        <div class="dash-product-item">
            ...
            <div class="dash-actions">
                <button class="btn-icon edit" onclick="editProduct('${prod.id}')">✏️</button>
                <button class="btn-icon delete" onclick="deleteProduct('${prod.id}')">🗑️</button>
            </div>
        </div>
    `).join('');
}
...
function saveProduct() {
    ...
    if (!imageUrl) {
        ...
        if (editingProductId) {
            ...
            imageUrl = 'https://via.placeholder.com/300x200?text=Sweet+Haven'; // Default placeholder
        }
    }
    ...
    if (editingProductId) {
        ...
        if (index !== -1) {
            products[index] = { ...products[index], name, price: Number(price), desc, offer, image: imageUrl };
        }
    } else {
        // Add new
        const newId = 'p' + Date.now();
        products.push({ id: newId, name, price: Number(price), desc, offer, image: imageUrl });
    }

    localStorage.setItem('sweetHavenProducts', JSON.stringify(products));
    clearForm();
    renderDashboardProducts();
    renderStoreProducts(); // Update main site immediately
...
function editProduct(id) {
    ...
    document.getElementById('prodOffer').value = prod.offer || '';
    
    const preview = document.getElementById('imagePreview');
    preview.src = prod.image;
    preview.style.display = 'block';
    base64Image = ''; // Reset base64 so we know if a new image is uploaded
